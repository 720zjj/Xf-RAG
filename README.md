...
型号过滤采用两层规则：涉及产品能力、参数、具体菜单、按键组合、恢复设置和教学视频时，不会退回到其他型号；涉及无法开机、无声、充电、发热、联网故障、翻译不准等常见设备问题时，如果当前型号没有直接答案，可以采用另一首发型号资料中不依赖型号的外部检查步骤，并以“通用设备排查”提示用户。相近但不直接覆盖问题的片段仍不能用于凑答案。
