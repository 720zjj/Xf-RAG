...
Xf-RAG 将文档切分、向量化并与元数据一起保存。问答时先过滤无权限、未审核和已废弃内容，再按问题类型应用产品范围：功能、参数、菜单、按键和视频严格限定型号；无法开机、无声、充电、发热、联网故障等非侵入式通用排查，在当前型号没有直接资料时可谨慎采用另一首发型号文档中的直接答案，并明确提示型号边界。没有可靠直接资料时，系统仍会说明未命中。
...
- **分层产品资料边界**：功能、参数、菜单、按键和视频严格按型号隔离；常见非侵入式故障允许使用带边界提示的通用排查，且只接受文档中的直接证据。
- **SOP 与视频推荐**：根据问题关键词推荐已审核 SOP 和已发布操作视频；官方 H5 原地址保留用于溯源，已核验的双屏 2.0 视频可另用 H.264 兼容地址在手机页面内播放；支持视频章节和解决状态记录。
...
### 1. 有直接证据先回答，精确细节保留型号边界

用户使用口语化、宽泛问法或没有重复写出扫码型号时，只要当前有效资料已经直接说明，系统就优先回答。型号过滤采用两层规则：具体参数、语种数量、菜单路径、按键组合、恢复设置、专属功能和教学视频保留资料中的型号范围；无法开机、无声、充电、发热、联网故障、翻译不准等非侵入式常见问题，在当前型号没有直接答案时可以采用另一首发型号资料中不依赖型号的检查步骤，并明确标注为通用排查。“支持哪些语言”按能力范围检索，“怎么切换语言”按操作步骤检索，两者不会互相代替。相近但没有直接回答问题的片段仍不能用于凑答案。
