...
test('识别支持语种范围问题，并与切换语种操作严格分开', () => {
  ...
  assert.equal(isSupportedLanguageCapabilityQuestion('英语能翻译吗？'), true)
  assert.equal(isSupportedLanguageCapabilityQuestion('支持英语翻译吗？'), true)
  assert.equal(isSupportedLanguageCapabilityQuestion('可以翻译英文吗？'), true)
  assert.equal(isSupportedLanguageCapabilityQuestion('日语能不能翻译？'), true)
  assert.equal(isSupportedLanguageCapabilityQuestion('怎么切换翻译语言？'), false)
  assert.equal(isSupportedLanguageCapabilityQuestion('英语不能翻译怎么办？'), false)
  ...
  assert.equal(getDirectSupportIntent('英语能翻译吗？'), 'supported-language-capability')
})

test('支持语种证据必须包含能力数量或语言清单，不能拿切换路径代替', () => {
  assert.equal(isSupportedLanguageCapabilityEvidence('双屏翻译机 2.0 支持 80 多种外语在线翻译。'), true)
  assert.equal(isSupportedLanguageCapabilityEvidence('在线翻译：支持80多个语种翻译，覆盖全球200多个国家和地区。'), true)
  assert.equal(isSupportedLanguageCapabilityEvidence('离线翻译支持中文普通话与 17 种语言互译：英语、日语、韩语。'), true)
  assert.equal(isSupportedLanguageCapabilityEvidence('离线翻译搭载离线大模型，可翻译包括英语、俄语、德语、法语在内的18种语言。', '英语能翻译吗？'), true)
  assert.equal(isSupportedLanguageCapabilityEvidence('离线翻译支持中文普通话与 17 种语言互译：英语、日语、韩语。', '英语能翻译吗？'), true)
  assert.equal(isSupportedLanguageCapabilityEvidence('离线翻译支持中文普通话与 17 种语言互译：英语、日语、韩语。', '德语能翻译吗？'), false)
  assert.equal(isSupportedLanguageCapabilityEvidence('打开语音翻译，在高级设置中调整语速；该功能仅支持中英文。', '英语能翻译吗？'), false)
  assert.equal(isSupportedLanguageCapabilityEvidence('支持四种方言，可将方言翻译为普通话或英语。', '英语能翻译吗？'), false)
  assert.equal(isSupportedLanguageCapabilityEvidence('在主界面选择翻译语种（如中文与英语），长按翻译键开始。', '英语能翻译吗？'), false)
  ...
  assert.equal(isDirectSupportEvidence('英语能翻译吗？', '离线翻译支持中文普通话与 17 种语言互译：英语、日语、韩语。'), true)
  assert.equal(isDirectSupportEvidence('德语能翻译吗？', '离线翻译支持中文普通话与 17 种语言互译：英语、日语、韩语。'), false)
})
