...
test('扫码渠道码覆盖全部现有问答端点，管理入口只向普通模式管理员展示', () => {
  ...
  assert.match(source, /const supportApiFetch = async \(path, options\) =>/)
  assert.match(source, /response\.status !== 401/)
  assert.match(source, /setSupportChannel\(renewedData\.data\)/)
  assert.match(source, /response = await fetch\(path, options\)/)
